...
struct Node *esquerdaRotate(struct Node *x) { 
    struct Node *y = x->direita;    // y é o filho à direita de x
    struct Node *T2 = y->esquerda;  // T2 é o filho à esquerda de y (realocado)

    // Passo 2: Realizar a rotação
    y->esquerda = x;       // Faz x ser o filho à esquerda de y
    x->direita = T2;       // Realoca T2 como filho à direita de x

    // Passo 3: Atualizar as alturas
    x->altura = max(altura(x->esquerda), altura(x->direita)) + 1;
    y->altura = max(altura(y->esquerda), altura(y->direita)) + 1;

    // Passo 4: Retornar a nova raiz
    return y;
} 
